export class World {
	...
	checkImpact(object: GameObject[]) {
		object.forEach((o) => {
			if ((o.health && o.health < 1) || o.alive === false) o.onDeath();
		});
	}
}
...
export class GameObject {
	...
	weight: number = 10;
	velocity: Vector2d = { x: 0, y: 0 }; // пикселей в секунду
	...
	updatePhysics(dt: number) {
		if (this.deathTimer !== null) {
			this.deathTimer += dt * 1000; // dt обычно в секундах
			...
			// Никакой физики больше для "умирающего" объекта
			return;
		}
	...
	takeDamage(ammount: number) {
		if (this.health && this.health !== null) {
			this.health -= ammount;
			console.log(`=> ${this.id} take damage`);
			if (this.health < 1) {
				this.alive = false;
				this.onDeath();
			}
	...
	updateAnimation(dt: number, frameDuration: number = 100) {
		...
		if (this.frameTimer > frameDuration) {
			...
			this.currentAnimationFrame =
				((this.currentAnimationFrame ?? 0) + 1) % frames.length;
			this.frameTimer = 0;
		}
	}

	onDeath() {
		console.log(`=> death of the: ${this.id}`);
...
export class Projectile extends GameObject {
	...
	updatePhysics(dt: number): void {
		super.updatePhysics(dt); // движение по X

		// Удаление, если вылетел за пределы canvas
		// canvas обычно доступен через world.canvas или передавай ссылку
		const cw = this.world.canvas?.width || 800; // если canvas нет в world, сделай поле canvas и пробрось его туда
		const ch = this.world.canvas?.height || 600;
		if (
			this.position.x > cw ||
