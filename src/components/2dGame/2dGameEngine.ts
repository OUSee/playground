...
export const Use2dGameEngine = () => {
	...
	const testGameClasses = async (ctx: any, canvas: any, debug: boolean) => {
		...
		foe.position = { x: 150, y: 0 };
		foe.color = 'orange';
		world.objects.push(foe);

		const floor = new GameObject('floor', world);
		floor.width = 300;
		floor.height = 30;
		floor.position = { x: 0, y: canvas.height - 30 };
		floor.weight = -1;
		floor.color = 'brown';
		...
		function draw() {
			...
			world.objects.forEach((obj) => {
				if (obj.sprite && obj.animations) {
					const frame =
						obj.animations[obj.currentAnimationKey ?? 'idle'][
							obj.currentAnimationFrame ?? 0
						];
					ctx.drawImage(
						obj.sprite,
						frame.x,
		...
		function gameLoop() {
			...
			// Здесь обработка ввода, прыжков, выстрелов и т.п.
			player.jump();
